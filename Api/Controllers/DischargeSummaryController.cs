...
namespace dotnet_admin_dashboard.Api.Controllers
{
    ...
    public class DischargeSummaryController : ControllerBase
    {
        ...
        [HttpPost("dto")]
        public async Task<IActionResult> CreateDto([FromBody] DischargeSummaryDto dto)
        {
            await _service.AddAsync(dto);
            return CreatedAtAction(nameof(GetDto), new { id = dto.Id }, dto);
        }
        [HttpPut("dto/{id}")]
        public async Task<IActionResult> UpdateDto(int id, [FromBody] DischargeSummaryDto dto)
        {
            dto.Id = id;
        ...
        [HttpPost("mongo")]
        public async Task<IActionResult> AddMongo([FromBody] DischargeSummary entity)
        {
            await _service.AddMongoAsync(entity);
            return CreatedAtAction(nameof(GetByIdMongo), new { id = entity.Id }, entity);
        }
        [HttpPut("mongo/{id}")]
        public async Task<IActionResult> UpdateMongo(string id, [FromBody] DischargeSummary entity)
        {
            entity.Id = id;
